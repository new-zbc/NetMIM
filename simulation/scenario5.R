...
#### missing gene expression and missing methylation ##
...
result_wd = "simulation/scenario5"

ratioE = c(0.1, 0.4, 0.25)
ratioM = c(0.4, 0.1, 0.25)
...
for(p in c(100, 200, 500)){
  ...
  mydata1 <- foreach(i=1:(2*n.cluster), .combine=rbind, .noexport = c('<Functions that were implemented in C++>')) %dopar%
    {
      ...
      sampleGene = sample(sampleid, size=n*ratio1)
      
      sampleMet = setdiff(sampleid, sampleGene)
      
      data_sim$train$E[sampleGene, ] <- NA
      
      data_sim$train$M[sampleMet, ] <- NA
  ...
  filename = paste0(result_wd, "/", "graphIBAG", "_ratio1_", ratio1,"_ratio2_", ratio2, "_dim_", p, ".txt")
  ...
  filename = paste0(result_wd, "/", "graphIBAG_0", "_ratio1_", ratio1,"_ratio2_", ratio2, "_dim_", p,".txt")
  ...
  mydata3 <- foreach(i=1:(2*n.cluster), .combine=rbind, .noexport = c('<Functions that were implemented in C++>')) %dopar%
    {
      ...
      data$geneObs <- data_sim$E[-sampleGene, ]
      row.names(data$geneObs) <- 1:dim(data$geneObs)[1]
      
      
      data$methyObs <- data_sim$M[-sampleMet, ]
  ...
  filename = paste0(result_wd, "/", "FBM", "_ratio1_", ratio1,"_ratio2_", ratio2, "_dim_", p, ".txt")
  ...
  # mydata1 <- foreach(i=1:(2*n.cluster), .combine=rbind, .noexport = c('<Functions that were implemented in C++>')) %dopar%
  #   {
  #     source("graphIBAG/iBAG_final.R")
  #     
  #     n.burnin = 2000
  #     max_iters = 10000
  #     data_sim = simData(n, gamma, Z, map, MethylSample1, sd=1, seed = i^2)
  #     
  #     sampleid = sample(n, size = n*ratio)
  #     
  #     data_sim$Y <- data_sim$Y[-sampleid]
  #     data_sim$C <- data_sim$C[-sampleid, ]
  #     data_sim$E <- data_sim$E[-sampleid, ]
  #     data_sim$M <- data_sim$M[-sampleid, ]
  #     
  #     result = GraphIBAG(data_sim, prior = prior, max_iters = max_iters)
  #     selection.perf(colMeans(result$gamma[n.burnin:max_iters, ]), gamma)
  #     #selection2 = selection.perf(colMeans(result$Z[n.burnin:max_iters, ]), Z)
  #     #c(selection1, selection2)
  #     
  #   }
  # stopCluster(cl)
  # 
  # filename = paste0(result_wd, "/", "graphIBAG_cc", "_ratio1_", ratio1,"_ratio2_", ratio2, "_dim_", p, ".txt")
  # write.table(mydata1, file = filename, quote = FALSE, row.names = FALSE)
  # 
  # vec1 = colMeans(mydata1, na.rm = T)
  # vec2 = apply(mydata1, 2, sd)
  # 
  # 
  # print(vec1)
  # print(vec2)
  ...
  # stopCluster(cl)
  # 
  # filename = paste0(result_wd, "/", "graphIBAG_0_cc", "_ratio1_", ratio1,"_ratio2_", ratio2, "_dim_", p,".txt")
  # write.table(mydata2, file = filename, quote = FALSE, row.names = FALSE)
  # 
  # vec1 = colMeans(mydata2, na.rm = T)
  # vec2 = apply(mydata2, 2, sd)
  # 
  # 
  # print(vec1)
  # print(vec2)
  ...
  #     
  #     result = FBMcpp(data, max_iters, 15217, "Full")
  #     selection.perf(colMeans(cbind(result$IMbar[n.burnin:max_iters, ], result$IM[n.burnin:max_iters, ])), gamma)
  #     
  #   }
  # stopCluster(cl)
  # 
  # filename = paste0(result_wd, "/", "FBM_cc", "_ratio1_", ratio1,"_ratio2_", ratio2, "_dim_", p, ".txt")
  # write.table(mydata3, file = filename, quote = FALSE, row.names = FALSE)
  # 
  # vec1 = colMeans(mydata3, na.rm = T)
  # vec2 = apply(mydata3, 2, sd)
  # 
  # 
  # print(vec1)
  # print(vec2)
...
colnames(res.sum) = c("sens", "spec", "MCC", "AUC")
filename = paste0(result_wd, "/", "summary_ratio1.csv")
