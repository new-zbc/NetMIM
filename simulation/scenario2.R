...
#### predict response of test data with penalty method
predict_Y_penalty_method <- function(result, data){
  Y = data$Y
  C = data$C
  E = data$E
  M = data$M
  ...
  for(k in 1:p){
    E_hat[ ,k] = as.matrix(M[, which(map == k)]) %*% result$Omega[which(map == k)]
  }
  
  output = C %*% result$beta_c + cbind((E - E_hat), E_hat) %*% result$beta
...
result_wd = "simulation/scenario2_model2"
...
for(p in c(100,200,500)){
  for(err_sd in c(1, 2, 3)){
    ...
    # generate gamma
    gamma_ground_truth = c(1:20, p+(1:10), p+(21:30))
    gamma = rep(0, 2*p)
    ...
    mydata1 <- foreach(i=1:(2*n.cluster), .combine=rbind, .noexport = c('<Functions that were implemented in C++>')) %dopar%
    {
      ...
      n.burnin = 2000
      max_iters = 10000
      data_sim = simData(n+50, gamma, Z, map, MethylSample1, sd=err_sd, seed = i^2)
    ...
    filename = paste0(result_wd, "/", "graphIBAG", "_sd_", err_sd, "_dim_", p, ".txt")
    ...
    filename = paste0(result_wd, "/", "graphIBAG_0", "_sd_", err_sd, "_dim_", p,".txt")
    ...
    mydata3 <- foreach(i=1:(2*n.cluster), .combine=rbind, .noexport = c('<Functions that were implemented in C++>')) %dopar%
    {
      ...
      dataList = simData(n+50, gamma, Z, map, MethylSample1, sd=err_sd, seed = i^2)
      ...
      data$geneObs <- data_sim$E
      row.names(data$geneObs) <- 1:dim(data_sim$E)[1]
      
      
      data$methyObs <- data_sim$M
      data$methyObs <- as.matrix(data$methyObs)
      row.names(data$methyObs) <- 1:dim(data_sim$E)[1]
    ...
    filename = paste0(result_wd, "/", "FBM", "_sd_", err_sd, "_dim_", p, ".txt")
    ...
    cl = makeCluster(n.cluster)
    registerDoParallel(cl)  
    mydata4 <- foreach(i=1:(2*n.cluster), .combine=rbind) %dopar% 
    {
      source("penalty/penalty_method.R")
      
      
      data_sim = simData(n+50, gamma, Z, map, MethylSample1, sd=err_sd, seed = i^2)
      
      result = lasso.sim(data_sim$train, gamma, seed = 1157)
      
      hatY = predict_Y_penalty_method(result, data_sim$test)
      mse = mean((hatY - data_sim$test$Y)^2)
      perf = selection.perf(result$gamma, gamma)
      
      c(perf, mse, result$gamma)
      
    }
    stopCluster(cl)
    
    filename = paste0(result_wd, "/", "lasso", "_sd_", err_sd, "_dim_", p, ".txt")
    write.table(mydata4, file = filename, quote = FALSE, row.names = FALSE)
    
    vec1 = colMeans(mydata4, na.rm = T)
    vec2 = apply(mydata4, 2, sd, na.rm = T)
    ...
    cl = makeCluster(n.cluster)
    registerDoParallel(cl)  
    mydata5 <- foreach(i=1:(2*n.cluster), .combine=rbind) %dopar% 
    {
      source("penalty/penalty_method.R")
      ...
      result = elastic.sim(data_sim$train, gamma, seed = 1157)
    ...
    filename = paste0(result_wd, "/", "elastic", "_sd_", err_sd, "_dim_", p, ".txt")
    write.table(mydata5, file = filename, quote = FALSE, row.names = FALSE)
    
    vec1 = colMeans(mydata5, na.rm = T)
    vec2 = apply(mydata5, 2, sd, na.rm = T)
...
filename = paste0(result_wd, "/", "summary_p_sd.csv")
