...
survivalNIBAMM <- function(data, init=NULL, prior, max_iters, seed=15267){
  set.seed(seed)
  gammaTimes = 40
  rho = 0.5
  ...
  # store the missing response
  Y_miss = matrix(NA, nrow = max_iters, ncol = n_censor)
  ...
  #### initialize Y_miss
  Y_miss[1, ] = Y_in[censorindex] + rexp(n_censor)
  ...
  if(n_exp_miss !=0 & n_met_miss !=0){
    ...
    for(i in 2:max_iters)
    {
      ...
      #### update Y_missing
      for(j in 1:n_censor){
        Y_miss[i,j] <- MH_update_Y(j, Y_in[censorindex], censorindex, Y, X, gamma[i*gammaTimes, ], Hbar, tau, delta1, delta2)
      }
        
      Y[censorindex] = Y_miss[i, ]
